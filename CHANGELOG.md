...
## Unreleased
- `approve()`/`auto_approve()` (--approve, --confirm-pick, `--answer` auto-cache) no longer fail outright when `search`'s own retrieval comes back short of "ready" for a file already picked or gated as evidence: `search` is a separate, narrower single-pointer engine from ask()'s own ranking (routing across every pointer + word-search fallback + listwise), so it can come back no-match/refused for a file ask() already listed first. Its attemptId is still recorded on a non-ready result, so the picked file is now cited directly via file_evidence's existing assist fallback instead of refusing to save. Fixes `--confirm-pick` "search returned no-match on <pointer>" on a file ask() had just ranked #1 (businessfi report, 2026-09-25).
