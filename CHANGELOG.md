...
## Unreleased
- Word search scores each file by its best 3500-character passage instead of the whole file, so a long note (the 36 KB medical timeline) is no longer sunk by its unrelated visits; a written phone number (212-305-6390) counts as the words "phone" and "number"; a file under half the best word score takes no read slot. Full eval (corrected key, 49 scored): top-1 41 -> 42, right-confirmed 34 -> 36, wrong-confirmed 2 -> 2; content-check reads 7.9 -> 7.2 per question. Fixes "NYP ENT phone number" (hand test 6).
- Stale pointers: when every file of a stale pointer is already reviewed at its current bytes (auto-heal used to skip it as "no-change" forever, e.g. primary-reference), the ask reconnects it inline with `--no-findability` (no writer or Jev calls, 45 s cap, then left running in the background) and reads it in the same lookup. Split parts (`<pointer>-N`) now heal through the parent's report instead of skipping as "no-report". Real changes keep the background refresh.
