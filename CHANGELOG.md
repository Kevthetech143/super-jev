...
- Pick trail: a refused pick (TIME_SENSITIVE/REJECT/CONTRADICTED/stale/secret-held) used to be dropped from `pending_picks.jsonl` with only a transient print during the automatic every-5-picks flush, so it vanished from `--pending-picks` with no lasting trace beyond `lookups.jsonl` -- a correct, "fair" refusal looked identical to a silent bug. It now stays listed with `dropped: <reason>`, exactly like an unsure/READ pick, until a human runs `--confirm-pick` or `--drop-pick`. (businessfi hand test, 2026-09-25)
