...
Two rules the planner will not bend. A call never exceeds `maxRecordsPerCall` whatever the token arithmetic says. A record whose own estimate exceeds the per-call record allowance is never packed; it lands in `oversizedRecordIds`, is never sent, and is routed to review. There is no "it will probably fit".
...
A dangling reference on its own is a warning rather than a block. When it is the reason a required role is empty, the role check already blocks; when the role was filled another way, the broken reference is a data-quality note that travels with the result.
...
| `reservedForQuestions` | 1500 | Option descriptions repeat in every record's question, so the question block grows with the batch, not with the corpus. |
