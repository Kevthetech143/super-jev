...
**Free checks first, judge for the rest.** Before any of the above, fetch runs three offline pre-rules (`src/enhance/fetch-prerules.ts`, ported from the offline-proven skill-pick research in `skillpick_facts.py`) that read the catalog's own `utterances`/`negatives` against the request in plain string matching — no provider call, no network. Each record's **trigger set** is built once from its `utterances`, normalized, with two exclusions: single-word utterances (too generic to trust blind) and any utterance whose exact text is also claimed by another record's `utterances` (the ambiguity guard — a phrase two records both use can't uniquely resolve either one). **R1** serves a record directly, with zero judge calls, when the request contains exactly one record's own unique multi-word trigger and none of that record's negatives; the manifest and cost both report zero calls, and the result carries `source: "trigger"`. **R2** demotes the judge's own top-1 pick to review when it has a negative-phrase hit in the request, and prints why. **R3** returns `noMatch` when no record's trigger appears anywhere in the request at all and the judge's top-1 confidence is under the floor. Pre-rules are **on by default**; `--no-prerules` skips straight to the plain judge + none-gate flow above. `--explain` prints the derived facts — which trigger hit, which negatives fired, how many utterances the ambiguity guard dropped — before the plan.
