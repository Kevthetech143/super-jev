...
|---|---|---|
| `ask` | `python3 ask.py --principal YOUR_AGENT "question in your own words"` | The loop: harness cache first, then every pointer in parallel; open the top file; `--approve` a good hit so the next ask is instant; `--add` a fact with no file (one small pointer per entry, earlier answers untouched), labelled the same kind/status/as_of/subject as bulk-onboarded files so `prepare_bulk.py --list --principal YOUR_AGENT` finds it too, with `--replace-entry` to re-add over an existing manual entry for the same wording; `--miss` when grep found it elsewhere. Errors are printed per pointer and never hidden as no-candidates. |
