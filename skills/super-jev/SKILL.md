...
**Many files? Bulk prepare:** `python3 prepare_bulk.py --root DIR --pointer NAME --principal YOUR_AGENT_NAME [--limit 50]` inventories the folder (skips hidden dirs, backups, vaults), holds back files with card/password-like text or over the size ceiling, has a cheap writer (`claude -p --model haiku`) draft one description and one sample question per file, gates every description with Jev (one rewrite retry), connects the passing set through the same checked path, then checks each file ranks first on its own question. Cache by file hash under `prepare-cache/` so unchanged files are skipped on rerun. Read the HELD and EXCEPTION lines; they are yours to resolve. Limits: 50 files per pointer, no automatic rerun on file change, sample-question findability is a soft check.
