...
## Pick trail (agent-pick+check)

After answering from a listed file, an agent (or an end-of-reply hook) runs `ask.py --principal P --used <lookup_id|last> --rank N [--answer "text"]` (or `--file PATH`). The pick queues in `$STATE/pending_picks.jsonl` and a `pick` line with its rank lands in `traces.jsonl` (recorded only; ranking does not use it yet). When `SUPERJEV_PICK_BATCH` (default 5) unchecked picks queue, or on `--flush-picks`, each answer is checked against the passage cited from its chosen file with the same gate as `--answer`: only CLEAN at 0.80+ on an unchanged file saves, as `approved_by: agent-pick+check`. REJECT/CONTRADICTED, TIME_SENSITIVE, stale and secret-held picks are dropped. Anything else, including a pick with no answer text, stays pending: `--pending-picks` lists them, `--confirm-pick ID ["answer"]` approves one as a human, `--drop-pick ID` removes it. `SUPERJEV_AUTO_CACHE=0` keeps every pick queued unchecked.
